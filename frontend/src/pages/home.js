        ...
        <div className='text-center bg-[#ACD7EC] min-h-screen items-center justify-center py-[15px]'>
            ...
            <ul>
                {breakfastItems.map(foodItem => (
                    <li key={foodItem._id} className='my-[5px]'>{foodItem.name} - {foodItem.calories} cal <button className='rounded-full bg-[#F1AB86] px-[12px] py-[3px] font-medium' onClick={() => removeFoodItem(foodItem, foodItem.uniqueID)}>Remove Item</button></li>
                ))}
            </ul>
            ...
            <ul>
                {lunchItems.map(foodItem => (
                    <li key={foodItem._id}>{foodItem.name} - {foodItem.calories} cal <button className='rounded-full bg-[#F1AB86] px-[18px] py-[6px] font-medium' onClick={() => removeFoodItem(foodItem, foodItem.uniqueID)}>Remove Item</button></li>
                ))}
            </ul>
